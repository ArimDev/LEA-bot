        ...
        <thead>
            <tr>
                <th scope="col" rowspan="2" style="text-align: center;">
                    <img class="tablesBadge" src="../assets/logo/SAND.png" alt="Odznak SAND" /> <!-- MĚNIT VELIKOST NA TELEFONU -->
                </th>
        ...
        <tbody>
            <template v-for="(staff, index) in staffData" :key="index">
                ...
                <tr v-for="slotIndex in staff.slots" :key="`${index}-${slotIndex}`" :id="getCallsign(staff.firstCallsign, slotIndex)">
                    ...
                    <td class="tdLeft">
                        <template v-if="(workerData = getWorker(getCallsign(staff.firstCallsign, slotIndex)))">
                            {{ workerData.name || "" }}
                        </template>
                        <template v-else>
                            {{ "" }}
...
async function fetchWorkers() {
    try {
        const response = await axios.get("/api/v1/db/getTable/SAND", {
...
async function fetchPositions() {
    try {
        const response = await axios.get("/api/v2/db/SAND/positions");
...
function getWorker(callsign) {
    if (!callsign) return null;
    else return workers.value.find(worker => worker.radio === callsign) || null;
}
